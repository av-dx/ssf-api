    ...
    def analyzeNode(self, text):
        ...
        if acrefMod != None:
            ...
            for i in acrefModtemp:
                ...
                if self.checkuniqueEntityinmodelist(self.upper.upper.acoreferenceEntityNodeList, uniqid):
                    unique = self.getuniqueEntityinmodelist(
                        self.upper.upper.acoreferenceEntityNodeList, uniqid)
                else:
        ...
        if acrefModhead != None:
            ...
            for i in acrefModheadtemp:
                modeid = i.split(':')[1]
                modehead = i.split(':')[0]
                unique = self.getmodeinmodelistbymode(
                    self.upper.upper.acoreferenceEntityNodeList, modeid)
            ...
            for i in creftemp:
                ...
                if not self.checkforchainlist(self.upper.upper.coreferenceChainNodeList, chainid):
                    ...
                    if not self.checkuniqueEntityinmodelist(self.upper.upper.acoreferenceEntityNodeList, uniqid):
                        unique = Coreference_entity(
                            uniqid)  # new entity creation
                    ...
                    # added entity to chainnodelist
                    chain.nodeList.append(unique)
                    self.upper.upper.coreferenceChainNodeList.append(
                        chain)  # added chain to doc nodelist

                else:
                    chain = self.getchainfromchainlist(
                        self.upper.upper.coreferenceChainNodeList, chainid)  # get chain from nodelist
                    if not self.checkforentityinchainlist(chain, uniqid):
                        ...
                        unique = self.getentityfromchainlist(
                            chain, uniqid)  # get entity from
                        unique.hindistring += ' ' + self.lex
                        unique.endlineno = self.linenumber
                unique.lines.append(self.linenumber)
                unique.nodes.append(self)
        ...
        if crefHead:
            ...
            for i in crefHeadtemp:
                head = i.split(':')[0]
                uniqid = i.split(':')[1]
                unique = self.getentity(
                    self.upper.upper.coreferenceChainNodeList, uniqid)
            ...
            if crefType:
                ...
                for i in crefTypetemp:
                    ...
                    unique.parent = self.getentity(
                        self.upper.upper.coreferenceChainNodeList, cuniqueid)
        ...
        if crefChainHead:
            ...
            for i in crefChainHeadtemp:
                ...
                chainid = i.split(':')[1]
                chain = self.getchainfromchainlist(
                    self.upper.upper.coreferenceChainNodeList, chainid)
    ...
    def printSSFValue(self, prefix, allFeat):
        ...
        for node in self.nodeList:
            ...
            if isinstance(node, ChunkNode):
                returnStringList.extend(node.printSSFValue(
                    prefix + '.' + str(nodePosn), allFeat))
            else:
                returnStringList.append(node.printSSFValue(
                    prefix + '.' + str(nodePosn), allFeat))
        returnStringList.append('\t' + '))')
                ...
                if len(splitLine) > 0 and splitLine[0] == '))':
                    ...
                    if currentChunkNode.getAttribute('af') != None:
                        aa = currentChunkNode.getAttribute(
                            'af').strip().split(',')
                        if len(aa) >= 6:
                            currentChunkNode.morphroot = aa[0]
                            currentChunkNode.morphPOS = aa[1]
                            currentChunkNode.number = aa[3]
                            currentChunkNode.person = aa[4]
                            currentChunkNode.gender = aa[2]
                            currentChunkNode.case = aa[5]
                            currentChunkNode.tamUtf = aa[6]
                            currentChunkNode.tamWx = aa[7]
                    ...
                    if len(splitLine) > 100000000000000:
                        chunk_split = ''.join([x for x in splitLine if 'chunkType=' in x]).replace(
                            '>', '').split('=')[1].replace("'", "").split(':')
                        chunk_name = chunk_split[1]
                        chunk_relation = chunk_split[0]
                        currentNode_ext = Node_vandan(
                        ...
                        if previous_chunk_name != chunk_name:
                            ...
                            currentChunkNode.upper = lastContext
                            currentChunkNode.upper.nodeList.append(
                                currentChunkNode)
                            if currentChunkNode.upper.__class__.__name__ != 'Sentence':
                                currentChunkNode.upper.text.append(line)
                        else:
            ...
            for j in i.nodeList:
                ...
                if drel == None:
                    ...
                    if dmrel == None:
                        ...
                        j.parent = self.find_dependency_parent_node(
                            lastContext.nodeList, d_temp[1])
    ...
    def printSSFValue(self, allFeat=True):
        ...
        if self.nodeList != []:
            ...
            for node in nodeList:
                ...
                # nodePosn += 1
                nodewisestring, nodePosn = node.printSSFValue(
                    str(nodePosn), allFeat, nodePosn)
    ...
    def analyzeDocument(self):
        ...
        for sentence in sentenceList:
            tree = Sentence(sentence, self, ignoreErrors=True,
                            nesting=True, dummySentence=False)
            tree.upper = self
